...
export const thunk = {
  ...
  setCurrentTime: (val) => {
    return (dispatch, getState) => {
      ...
      if (val!=0 && val>=audioDuration-0.06) {
  ...
  onButtonClick: (object) => {
    return (dispatch, getState) => {
      ...
      if (timestampsForAttempts[branch]) {
        ...
        // console.log("form index.js"+key) // for debug
        // console.log("form index.js"+object.branch) // for debug
        setTimeout(() => {
        ...
        if (attemptObject) {
          ...
          dispatch(thunk.setUpdatedCurrentTime(time+ 0.250, true)); //changed
          dispatch(togglePlay(true));
          dispatch(togglePlayButton(true));
          // console.log("time form index.js"+time) // for debug
        }
      }
    };
  },
  ...
  restartAudio: () => {
    return (dispatch, getState) => {
      dispatch(thunk.setUpdatedCurrentTime(0.1 + .100));
      dispatch(togglePlay(true));
      dispatch(togglePlayButton(true));
      setTimeout(() => {
