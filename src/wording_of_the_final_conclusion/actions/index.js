...
export const thunk = {
  ...
  setCurrentTime: (val) => {
    return (dispatch, getState) => {
      ...
      if (val!=0 && val>=audioDuration-0.05) {
  ...
  selectNextFrame: () => {
    return (dispatch, state) => {
      ...
      if (index < tocIds.length - 1) {
        if(tocData[index+1].hidden) {
          dispatch(thunk.setToc(tocIds[index + 2]));
          setTimeout(() => {
            dispatch(
              updateLiveMessage(
                altText.frameButton.next + " " + tocData[index + 2].name
              )
            );
          }, 100);
        } else {
  ...
  onContinueQuestion: () => {
    return (dispatch, getState) => {
      ...
      if (question && question.onContinueTimestamp) {
        dispatch(thunk.setUpdatedCurrentTime(question.onContinueTimestamp+0.250));
      }
      setTimeout(() => {
  ...
  restartAudio: () => {
    return (dispatch, getState) => {
      dispatch(thunk.setUpdatedCurrentTime(0.1 + .350));
      dispatch(togglePlay(true));
      dispatch(togglePlayButton(true));
      setTimeout(() => {
