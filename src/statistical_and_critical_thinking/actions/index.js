...
// import audioPause from "../data/audioPause";
...
export const thunk = {
  ...
  setCurrentTime: (val) => {
    return (dispatch, getState) => {
      ...
      // for (let i = 0; i < pauseTimeStamp.time )
      // const pauseAudio = audioPause.pauseTimeStamp.filter(
      //   (e) => e.startTime <= val && e.endTime >= val
      // );
      // if (pauseAudio.length > 0) {
      ...
      //       dispatch(setUpdatedCurrentTime(pauseAudio[0].endTime));
      ...
      if (val != 0 && val >= audioDuration-0.25) {
  ...
  restartAudio: () => {
    return (dispatch, getState) => {
      dispatch(thunk.setUpdatedCurrentTime(0.1 + 0.250));
      dispatch(togglePlay(true));
      dispatch(togglePlayButton(true));
      setTimeout(() => {
