...
export const thunk = {
  ...
  setCurrentTime: (val) => {
    return (dispatch, getState) => {
      const {
        ...
        audioAutoPaused: aap,
        question: _question,
        attemptedFlowchartPath: _attemptedFlowchartPath,
        audioCurrentTime,
        audioDuration
      } = getState();
      ...
      const ost = onScreenText.filter((e) => e.startTime <= audioCurrentTime && e.endTime > audioCurrentTime && e.type == "message")[0];
      if (ost && ost.value && !_attemptedFlowchartPath) {

        const getKeys = Object.keys(ost.value);

        const getLastItem = getKeys[getKeys.length - 1];

        dispatch(attemptedFlowchartPath(getLastItem));
  ...
  setUpdatedCurrentTime: (time, selectedExample) => {
    return (dispatch, state) => {
      const {
        tocData,
        audioAutoPaused: aPaused,
      ...
      if (!selectedExample) {
        const ost = onScreenText.filter((e) => e.startTime <= time && e.endTime > time && e.type == "message")[0];
        // const getExample=timestampsForAttempts['condition1'].filter((e) => e.startTime <= time && e.endTime);
    ...
    return (dispatch, getState) => {
      ...
      if (timestampsForAttempts[branch]) {
        ...
        if (attemptObject) {
          ...
          const selectedQuestion = timestampsForAttempts[branch].filter((e) => e.id == id)[0];



          const time = selectedQuestion[key];

          if (selectedQuestion && selectedQuestion["correct"] == key) {
            // correct attempt, answer matched

            dispatch(onInteractiveSectionButtonClick({ ...object }));
            dispatch(recordedAnswer({ ...object }));
            // dispatch(setUpdatedCurrentTime(timestampsForAttempts[branch][key]));
            dispatch(attemptedFlowchartPath(""));
          } else {
            //alert(key);
          ...
          dispatch(thunk.setUpdatedCurrentTime(time + 0.250, true));
          dispatch(togglePlay(true));
          dispatch(togglePlayButton(true));
        }
      }
  ...
  restartAudio: () => {
    return (dispatch, getState) => {
      dispatch(thunk.setUpdatedCurrentTime(0.1 + .250));
      dispatch(togglePlay(true));
      dispatch(togglePlayButton(true));
      setTimeout(() => {
...
export const setFlowChartVisible = (value) => ({
  type: Types.SET_FLOW_CHART_VISIBLE, value
})